...
5. **AI服务 (AI Service)**
   - 模型调度管理（集成LightRAG、CrewAI、LangGraph）
   - 智能推荐
   - 语法检查
   - 技术栈：FastAPI + LightRAG + CrewAI + LangGraph + sentence-transformers
...
class AIModelGateway:
    ...
    async def _initialize_models(self):
        ...
        self.crew_agents = {
            ...
            'grammar_checker': AssistantAgent(
                name="grammar_checker",
                system_message="You are an English grammar expert."
            )
        }
    ...
    async def translate(self, request: TranslationRequest) -> TranslationResponse:
        ...
        try:
            ...
            if request.use_rag:
                # 使用LightRAG增强翻译
                context = await self.lightrag.query(request.source_text)
...
# 集成LightRAG、CrewAI和LangGraph的增强服务
class EnhancedAIService:
    def __init__(self):
        self.lightrag = LightRAG(working_dir="./rag_storage")
        self.crew_manager = self._setup_crewai()
        self.langgraph_workflow = self._setup_langgraph()
